...
def raster(v, f, vn, depth, normal, mutex):
    ...
    iMax = min(math.floor(max(p1[0],p2[0],p3[0]))+1, res) # x+1
    jMax = min(math.floor(max(p1[1],p2[1],p3[1]))+1, res) # x+1
...
def work(name, res, vnum):
    ...
    normal, depth = r.render(R)
    
    normal = (normal+1)*127.5
    normal = normal[:,:,::-1]
    cv2.imwrite("n.png", normal)
    
    depth = (depth+1) * 127.5
    cv2.imwrite("d.png", depth)

if __name__ == "__main__":
    work("/data/fq/data/mesh/0000/mesh.npz", 512, 64)